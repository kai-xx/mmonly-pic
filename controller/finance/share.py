...
class GetList:
    ...
    def getHtml(self, url, page):
        ...
        try:
            self.wait.until(
                EC.presence_of_element_located((By.CSS_SELECTOR, '.list-main')))
            self.html = self.brower.page_source
            self.fatHtml = pq(self.html)
            items = self.fatHtml(".list-main").children().children().items()
            lists = []
...
class GetDetail:
    ...
    def handleContent(self, html, tool, title):
        ...
        kline = imgSoap.find_all("div", "kline")
        if (len(kline) > 0):
            del imgSoap.find_all("div", "kline")[0]
