...
"""
获取贵金属
http://gold.jrj.com.cn/list/sckx.shtml
"""
...
class GetList:
    ...
    def getHtml(self, url, page):
        ...
        try:
            self.wait.until(
                EC.presence_of_element_located((By.CSS_SELECTOR, '.p10 .newslist5')))
            self.html = self.brower.page_source
            self.fatHtml = pq(self.html)
            items = self.fatHtml(".p10 .newslist5").children().items()
            lists = []
            ...
            for item in items:
                if self.count > getPageNumber.getCount():
                    ...
                    exit()
                title = item.children().eq(0).text()
                if not title:
                    continue
                href = item.children().eq(0).attr.href
    ...
    def waitForGetAllData(self):
        ...
        if self.html == None:
            return
        items = self.fatHtml(".jrj-pagenum").children('a')
        if not items:
            self.isPaging = False
...
class GetDetail:
    ...
    def getHtml(self):
        ...
        title = fatHtml(".titmain").children('h1').text()
        info = fatHtml(".titmain .inftop").children('span').eq(0).text()
        ...
        # 导航有多种格式 不同样式先发现
        categorysHtml = fatHtml(".cbox").children('p').children('a').items()
        categorys = []
        i = 0
